...
contract ChainFacet is Initializable, IERC721Receiver {
    ...
    function addStake(uint256 _budsAmount, uint256 _farmerTokenId) public {
        ...
        if(_farmerTokenId != 0 && LibGlobalVarState.mappingStore().stakedFarmer[msg.sender]) revert LibGlobalVarState.FarmerStakedAlready();
        ...
        if(_farmerTokenId != 0){
            ...
            LibGlobalVarState.mappingStore().stakedFarmer[msg.sender] = true;
    ...
    function boostStake(uint256 tokenId, uint256 stakeIndex) external {
        ...
        uint rewardAsPerApr = (stk.budsAmount * apr) / 10000;
        rewardAsPerApr = (rewardAsPerApr * 7)/365;
        if (len < 4) {
            LibGlobalVarState.mappingStore().boosts[msg.sender].push(block.timestamp + 7 days);
            uint256 amountBoosted = len == 1
                ? ((rewardAsPerApr / 100) * 5)
                : len == 2
                    ? ((rewardAsPerApr / 100) * 4)
                    : len == 3
                        ? ((rewardAsPerApr / 100) * 2)
                        : (rewardAsPerApr / 100);
    ...
    function claimRewards(uint256 stakeIndex) public {
        ...
        if (len == 0 || len < stakeIndex) revert LibGlobalVarState.NoStakeFound();
    ...
    function unStakeBuds(uint256 _budsAmount, uint256 stakeIndex) public {
        ...
        if (block.timestamp - LibGlobalVarState.mappingStore().stakeRecord[msg.sender][stakeIndex].timeStamp > 1 days) revert ("Claim first");
    ...
    function raidCustom(uint256 riskLevel, uint256 tokenId) external payable {
        if (LibGlobalVarState.interfaceStore()._narcToken.balanceOf(msg.sender) == 0) revert LibGlobalVarState.NotANarc();
